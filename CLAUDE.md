...
ITコストをE2Eで可視化するダッシュボードSaaS。Next.js 15 (App Router) + Clerk 認証 + Vercel デプロイ。
...
| `SES_WEBHOOK_SECRET` | 任意のランダム文字列（SES Webhook署名検証用） |
...
Ollama llama3.1:8b
    ↓ JSON スキーマ検証
{ productName, subtotal, expiryDate, currency, billingPeriodStart, billingPeriodEnd, autoRenew }
```
