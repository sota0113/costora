...
import type { CostItem, ServiceCost, MonthlyAmount, AwsCredentials, GithubCredentials, DatadogCredentials, GcpCredentials } from '@/lib/types'
...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  try {
    let result: ServiceCost
    switch (item.type) {
      case 'vercel':    result = await fetchVercel(item); break
      case 'aws':       result = await fetchAws(item); break
      case 'anthropic': result = await fetchAnthropic(item); break
      case 'openai':    result = await fetchOpenAI(item); break
      case 'resend':    result = await fetchResend(item); break
      case 'github':    result = await fetchGitHub(item); break
      case 'datadog':   result = await fetchDatadog(item); break
      case 'gcp':       result = await fetchGcp(item); break
      default:
        return NextResponse.json({ error: 'このサービスタイプはコスト取得に対応していません' }, { status: 400 })
    }
